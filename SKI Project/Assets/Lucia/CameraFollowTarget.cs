...
public class CameraFollowTarget : PivotBasedCameraRig
{
    ...
    [SerializeField]
    private float turnSpeed = 1; // How fast the rig will turn to keep up with target's rotation
    [SerializeField]
    private float rollSpeed = 0.2f;// How fast the rig will roll (around Z axis) to match target's roll.
    [SerializeField]
    private bool followVelocity = false;// Whether the rig will rotate in the direction of the target's velocity.
    [SerializeField]
    private bool followTilt = true; // Whether the rig will tilt (around X axis) with the target.
    [SerializeField]
    ...
    [SerializeField]
    private float targetVelocityLowerLimit = 4f;// the minimum velocity above which the camera turns towards the object's velocity. Below this we use the object's forward direction.
    [SerializeField]
    ...
    {
        //follows the targets position with no delay time, unlike the rotation
        //transform.position = target.position;
        //target.position = new Vector3(transform.position.x, transform.position.y, transform.position.z);

        if (Mathf.Abs(target.position.y - transform.position.y) >= deltaY)
        {
            //might need to lerp this
            //transform.position = target.position;
            adjustForYPosition();
        }
        else
    ...
    protected override void FollowTarget(float deltaTime)
    {
        ...
        // camera position moves towards target position with lerp:
    ...
    //Make me a coroutine!!!
    private void adjustForYPosition()
    {
